  ...
  async save({name, lastName, nodeId}) {
    try {
      ...
      let {data} = await this.axios.patch(
          `https://adv-vue.firebaseio.com/players/${nodeId}.json`, userData
        ); //zmienia tylko zmienione wartości, metoda put najpierw czyści obiekt, a później zmienia wartości
